%% function FC_TDPF:
% Full Temperature-Dependent Newton-Rhapson Power Flow Algorithm
% Runs a Newton-Rhapson type power flow augmented with temperature data.
% This is a fully-coupled algorithm with a single (large) Jacobian matrix.
% 
% SYNTAX:
%   [V,delta,T,bus,branch,hist] = FC_TDPF(bus,branch,varargin)
%
% INPUTS:
...
function [V,delta,T,bus,branch,hist] = FC_TDPF(bus,branch,varargin)
	...
	while ( iter < maxIter )
		...
		% Update state vector
		x = [delta(sets.delta); V(sets.V); T(sets.T)];
        ...
        if ( err <= tol )
            if (verbose)
            	disp('FC-TDPF Power Flow Converged.')
            end
            break;
        else
            % Perform update
    ...
    if timing
        ...
        % Display
        disp( '---Run Time---' );
        fprintf(1, '%0d FC-TDPF Iterations\n', iter);
